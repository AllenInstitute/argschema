...
def test_simple_extension_required():
    with pytest.raises(mm.ValidationError):
        example1 = {}
        ArgSchemaParser(input_data=example1, schema_type=SimpleExtension, args=[])


SimpleExtension_example_invalid = {"test": {"a": 5, "b": 1, "d": ["a", 2, 3]}}

SimpleExtension_example_valid = {"test": {"a": "hello", "b": 1, "d": [1, 5, 4]}}
...
def test_simple_extension_pass():
    mod = ArgSchemaParser(
        input_data=SimpleExtension_example_valid, schema_type=SimpleExtension, args=[]
    )
...
def test_simple_extension_write_pass(simple_extension_file):
    ...
    mod = ArgSchemaParser(
        input_data=SimpleExtension_example_valid, schema_type=SimpleExtension, args=args
    )
    assert mod.args["test"]["a"] == "hello"
    assert mod.args["test"]["b"] == 1
    assert mod.args["test"]["c"] == 10
...
def test_simple_extension_old_pass():
    mod = ArgSchemaParser(
        ...
        schema_type=SimpleExtensionOld,
        args=[],
    )
    assert mod.args["test"]["a"] == "hello"
    assert mod.args["test"]["b"] == 1
...
class RecursiveSchema(argschema.schemas.DefaultSchema):
    children = mm.fields.Nested(
        "self", many=True, metadata={"description": "children of this node"}
    )
    name = mm.fields.Str(
        default="anonymous", metadata={"description": "name of this node"}
    )
...
recursive_data = {
    "tree": {
        ...
        "children": [
            {"name": "child1"},
            {"name": "branch1", "children": [{"name": "subchild1"}, {}, {}]},
        ],
    }
}
...
def test_recursive_schema():
    mod = ArgSchemaParser(
        input_data=recursive_data, schema_type=ExampleRecursiveSchema, args=[]
    )
...
def bad_test_recursive_schema():
    with pytest.raises(mm.ValidationError):
        ArgSchemaParser(
            input_data=recursive_data, schema_type=BadExampleRecursiveSchema, args=[]
        )


class ModelFit(argschema.schemas.DefaultSchema):
    fit_type = argschema.fields.Str(metadata={"description": ""})
    hof_fit = argschema.fields.InputFile(metadata={"description": ""})
    hof = argschema.fields.InputFile(metadata={"description": ""})


class PopulationSelectionPaths(argschema.schemas.DefaultSchema):
    fits = argschema.fields.Nested(ModelFit, metadata={"description": ""}, many=True)
...
def david_data(tmpdir):
    ...
    dict_args = {
        "paths": {
            "fits": [
                {"fit_type": "test", "hof_fit": str(files[0]), "hof": str(files[1])},
                {"fit_type": "test2", "hof_fit": str(files[2]), "hof": str(files[3])},
            ]
        }
    }
    yield dict_args
...
def test_simple_description():
    d = {"a": "hello", "b": 1, "d": [1, 5, 4]}
    argschema.ArgSchemaParser(input_data=d, schema_type=MyShorterExtension, args=[])
