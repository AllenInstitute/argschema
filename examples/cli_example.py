...
class MyNestedSchema(DefaultSchema):
    a = Int(default=42, metadata={"description": "my first parameter"})
    b = Bool(default=True, metadata={"description": "my boolean"})


class MySchema(ArgSchema):
    array = NumpyArray(
        default=[[1, 2, 3], [4, 5, 6]], dtype="uint8", description="my example array"
    )
    string_list = List(
        List(Str),
    ...
    int_list = List(
        Int, default=[1, 2, 3], metadata={"cli_as_single_argument":True, "description":"list of ints"}
    )
