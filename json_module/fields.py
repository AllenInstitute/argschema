...
class NumpyArray(mm.fields.List):
    ...
    which will convert any numpy compatible set of lists into a numpy array after deserialization, 
    and convert it back to a list when serializing, if dtype is given (as a numpy.dtype)
    the array will be converted to the type, otherwise numpy will decide what type it should be. '''
...
class OutputFile(mm.fields.Str):
    '''OutputFile marshamallow.fields.Str subclass which is a path to a file location that
       can be written to by the current user (presently tested by opening a temporary file to that
       location)'''
...
class InputDir(mm.fields.Str):
    '''InputDir is  marshmallow.fields.Str subclass which is a path to a
       a directory that exists and that the user can access (presently checked with os.access)'''
    def _validate(self,value):
...
class InputFile(mm.fields.Str):
    '''InputDile is a marshmallow.fields.Str subclass which is a path to a file location
       which can be read by the user (presently passes os.path.isfile and os.access = R_OK)'''
    def _validate(self,value):
...
class OptionList(mm.fields.Field):
    '''OptionList is a marshmallow field which enforces that this field is one of a 
    finite set of options.
    OptionList(options,*args,**kwargs) where options is a list of json compatible options
    which this option will be enforced to belong'''
