...
class BasicOutputFile(ModuleParameters):
    output_file = OutputFile(required=True,metadata={'decription':'a simple output file'})
...
def test_relative_file_input():
    fp = open(input_file_example['input_file'],'w')
    fp.write("test")
    fp.close()
    mod = JsonModule(input_data = input_file_example, schema_type = BasicInputFile,args=[])
...
def test_output_file_relative():
    mod = JsonModule(input_data = output_file_example, schema_type = BasicOutputFile,args=[])
