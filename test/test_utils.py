...
class BaseballSituation(ArgSchema):
    ...
    bottom = fields.Bool(
        required=True, metadata={"description": "is it the bottom of the inning"}
    )
    score_home = fields.Int(
        required=True,
        metadata={"description": "home team score (non-negative)"},
        validate=(lambda x: x >= 0),
    )
    score_away = fields.Int(
        required=True,
        metadata={"description": "away team score (non-negative)"},
        validate=(lambda x: x >= 0),
    )
    outs = fields.Int(
        required=True,
    ...
    batter = fields.Nested(
        Player, required=True, metadata={"description": "who is batting"}
    )
    pitcher = fields.Nested(
        Player, required=True, metadata={"description": "who is pitching"}
    )


def test_schema_argparser_with_baseball():
    example_situation = {
        "batter": {"name": "Barry Bonds", "number": 25},
        "pitcher": {"name": "Roger Clemens", "number": 21},
    ...
    ArgSchemaParser(
        input_data=example_situation, schema_type=BaseballSituation, args=[]
    )
