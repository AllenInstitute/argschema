...
class MyNestedSchema(argschema.schemas.DefaultSchema):
    one = argschema.fields.Int(
        required=True, metadata={"description": "nested integer"}
    )
    two = argschema.fields.Boolean(
        required=True, metadata={"description": "a nested boolean"}
    )


class MySchema(argschema.ArgSchema):
    a = argschema.fields.Int(required=True, metadata={"description": "parameter a"})
    b = argschema.fields.Str(
        required=False,
        default="my value",
        metadata={"description": "optional b string parameter"},
    )
    nest = argschema.fields.Nested(
        MyNestedSchema, metadata={"description": "a nested schema"}
    )
...
def test_my_parser():
    input_data = {"a": 5, "nest": {"one": 7, "two": False}}
    MyParser(input_data=input_data, args=[])


class MyNestedSchemaWithDefaults(argschema.schemas.DefaultSchema):
    one = argschema.fields.Int(default=1, metadata={"description": "nested integer"})
    two = argschema.fields.Boolean(
        default=True, metadata={"description": "a nested boolean"}
    )
...
class MySchema2(argschema.ArgSchema):
    ...
    nest = argschema.fields.Nested(
        MyNestedSchemaWithDefaults, metadata={"description": "a nested schema"}
    )
...
def test_boolean_command_line(default, args, expected):
    input_data = {"a": 5, "nest": {"one": 7, "two": default}}
